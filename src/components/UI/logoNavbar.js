...
const LogoNavBar = ({ notOnePageSection, setMenuOpened }) => {
  ...
  // Animation
  const ScrollTopTransition = useTransition(showScrollTop, {
    from: { opacity: 0 },
    enter: { opacity: 1 },
    leave: { opacity: 0 },
  });
  ...
  // Animation
  const LogoNavBarTransition = useTransition(darkMode, {
    config: { duration: 200 },
  ...
  const renderLogo = () => {
    return LogoNavBarTransition( (item, key, props )=>
      item ? (
  ...
  return notOnePageSection ? (
    ...
    <>
      ...
      {ScrollTopTransition(( item, key, props ) =>
